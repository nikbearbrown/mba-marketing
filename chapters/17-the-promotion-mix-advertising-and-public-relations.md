...
```
I'm in Chapter 17 of building my AI Marketing Assistant. This
chapter taught: advertising types (informative, persuasive,
reminder, comparative); ad KPIs (reach — % of audience exposed;
frequency — average exposures per person; GRP — reach × frequency;
CPM — cost per thousand impressions; CTR — click-through rate; ROI/
ROAS); the advertising plan (brief → creative → media buy →
trafficking → measurement); push (manufacturer → channel →
consumer) vs. pull (consumer demand pulls product through channel)
strategies; PR functions (publicity, lobbying, investor relations,
issue management, crisis communications); ethics in advertising and
PR (FTC truth-in-advertising, FDA for health claims, child-
protection rules, native-advertising disclosure).

The chapter's anchor claim: most advertising fails in the brief,
not in the creative. A vague brief produces creative that needs
multiple rounds of revision and still misses the strategic intent;
a sharp brief produces creative that ships first round.

Add the **Advertising + PR Capability** to my assistant. Produce
three things.

1. **System-prompt insert: Ad Brief Drafting.** A markdown section
   specifying the structure of a sharp ad brief: target audience
   (specific, named); single-minded proposition (one sentence,
   not three); reason to believe; tone and brand guardrails;
   mandatories; the "this brief is done when..." test (you should
   be able to predict whether a creative concept is on-brief or
   off-brief from the brief alone).

2. **System-prompt insert: Push/Pull Decision + Crisis-Comms
   Playbook.** A markdown section specifying how the assistant
   recommends push vs. pull (push when distribution is the
   bottleneck; pull when consumer awareness is) and how it drafts
   a 24-hour crisis-comms playbook (first-hour silence rule;
   internal-first-then-external sequencing; named spokesperson;
   single-channel-first then expand; the "what we know, what we
   don't know, what we're doing" structure).

3. **Two live drafts:**
   - An ad brief for the brand's next campaign.
   - A crisis-comms playbook for the brand's most plausible
     crisis (data breach? product recall? founder controversy?
     supply-chain failure? customer-service viral moment? Pick the
     most plausible for your brand specifically).

Document both. The crisis-comms playbook is one of those documents
that sits in a folder until needed and then determines whether
the brand survives the moment.
```

---

**What this produces:** Two system-prompt sections + an actual ad brief + an actual crisis-comms playbook. The crisis playbook is operationally valuable — most brands don't have one until the crisis happens.

**How to adapt this prompt:**

- *For your own project:* The crisis scenario should match the brand's actual risk profile. A DTC brand's most likely crisis is different from a healthcare brand's; choose carefully.
- *For ChatGPT / Gemini:* Works as written.
- *For Claude Code:* Not the primary tool here.
- *For a Claude Project:* Two new sections. The ad brief and crisis playbook both go in the project artifacts.

**Connection to previous chapters:** Chapter 16's IMC core message + Chapter 17's ad brief should be aligned. If the brief's single-minded proposition isn't the IMC core message, one of them is off.
